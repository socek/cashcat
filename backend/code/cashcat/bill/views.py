...
class BillView(BaseView):
    ...
    def patch(self):
        ...
        Update bill data.
        """
        schema = BillSchema(partial=("uid", "type", "owner_uid"))
        bill = self.get_validated_fields(schema)
        self.bill_command().update_by_uid(
            self.request.matchdict["bill_uid"], {"name": bill.name}
        )
